...
// keep chosen files here (useful when uploading)
    let selectedFiles = [];
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Optional: allow dropping files onto the placeholder
        ...
        placeholder.addEventListener('dragleave', e => {
            placeholder.style.borderColor = '#cfcfcf';
        });
        placeholder.addEventListener('drop', e => {
            e.preventDefault();
            placeholder.style.borderColor = '#cfcfcf';
            const dtFiles = Array.from(e.dataTransfer.files || []);
            // Use same handler as input: create a fake event-like object with .files
            previewImages({ target: { files: dtFiles } });
        });
    });

// Update the actual input.files property to match selectedFiles using DataTransfer
    ...
    function previewImages(event) {
        ...
        newFiles.forEach(file => {
            ...
            reader.onload = function(e) {
                ...
                // insert before the upload placeholder so the placeholder stays on the right
                container.insertBefore(wrapper, uploadPlaceholder);
            };
            reader.readAsDataURL(file);
        });

        // update input.files to include all selected files
        updateInputFiles();
    }

// for debugging: expose selectedFiles in console
